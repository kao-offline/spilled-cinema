...
import { getPlaybackProgressColor, type ContinueWatchingItem, type NewEpisodeItem } from "../lib/home-personalization";
...
type HomePersonalizedRailProps =
  | {
      ...
      onRetry: () => void;
      onPlay: (item: NewEpisodeItem | ContinueWatchingItem) => void;
      compact?: boolean;
    }
  | {
...
function artworkFor(item: NewEpisodeItem | ContinueWatchingItem, preview?: EpisodePreview) {
...
function episodeCode(item: NewEpisodeItem | ContinueWatchingItem) {
...
function EmptyNewEpisodes({ freshness, onRetry }: { freshness: FreshnessState; onRetry: () => void }) {
  return (
    <div className="mx-4 flex min-h-20 items-center justify-between gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.035] px-4 sm:mx-10 lg:mx-16">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs leading-5 text-white/42">{freshness.message ?? "Newly added episodes will appear here."}</p>
        </div>
      </div>
      {!freshness.checking ? (
        <button type="button" onClick={onRetry} className="inline-flex min-h-11 shrink-0 items-center gap-2 rounded-full bg-white/9 px-4 text-xs font-bold text-white transition hover:bg-white/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70">
...
export function HomePersonalizedRail(props: HomePersonalizedRailProps) {
  ...
  const isContinue = props.kind === "continue-watching";
  const title = isContinue ? "Continue Watching" : "New Episodes";

  if (isContinue && props.items.length === 0) return null;

  return (
    <section className={clsx("ui-virtual-section", props.compact ? "py-1.5" : "py-2.5")} aria-labelledby={headingId} aria-busy={!isContinue && props.freshness.checking}>
      <div className={clsx("mb-2.5 flex items-end justify-between gap-3", props.compact ? "px-4" : "px-5 sm:px-10 lg:px-16")}>
        <h2 id={headingId} className="text-[17px] font-black tracking-[-0.025em] text-white sm:text-lg">{title}</h2>
        {!isContinue ? (
          <button
            type="button"
      ...
      {props.items.length === 0 && !isContinue ? <EmptyNewEpisodes freshness={props.freshness} onRetry={props.onRetry} /> : (
        <div className={clsx("no-scrollbar flex snap-x snap-mandatory gap-2.5 overflow-x-auto pb-2.5", props.compact ? "px-4" : "px-5 sm:px-10 lg:px-16")}>
          {props.items.map((item) => {
            ...
            const continueItem = item.kind !== "new-episode" ? item : null;
            const progress = continueItem?.progressPercent ?? 0;
            const action = item.kind === "new-episode"
              ? item.hasWatchHistory ? "Up next" : "Start series"
              : item.kind === "resume-film"
                ? `Resume at ${formatTime(item.resumeAtSeconds) ?? "start"}`
                : item.resumeAtSeconds > 0 ? `Continue at ${formatTime(item.resumeAtSeconds)}` : "Next episode";
            const date = item.kind === "new-episode"
              ? preview?.airDate ? formatAirDate(preview.airDate) : formatDate(item.addedAt)
              : null;
            ...
            return (
              <button
                ...
                aria-label={`${item.kind === "resume-film" ? "Resume" : "Play"} ${item.show.title}, ${formatEpisodeTitle(item.episode)}${date ? `, ${preview?.airDate ? "released" : "added"} ${date}` : ""}`}
              >
      ...
      {!isContinue && props.freshness.message && props.items.length > 0 ? (
