...
describe("responsive UI scenario matrix", () => {
  it.each([
    ...
    { name: "1080p TV", viewportWidth: 1920, memory: 8, economy: true, heroSize: "w780" },
    { name: "4K TV", viewportWidth: 3840, memory: 8, economy: true, heroSize: "w780" },
    { name: "data saver desktop", viewportWidth: 1366, memory: 8, saveData: true, economy: true, heroSize: "w780" },
  ])("selects the expected artwork tier for $name", ({ viewportWidth, memory, saveData, economy, heroSize }) => {
    const scenario = { viewportWidth, deviceMemoryGb: memory, saveData };
