...
const routes: Array<{ path: string; handler: RouteHandler }> = [
  ...
  { path: "/api/remote/pair", handler: handlers.remotePairHandler },
  { path: "/api/remote/unpair", handler: handlers.remoteUnpairHandler },
  { path: "/api/remote/info", handler: handlers.remoteInfoHandler },
  { path: "/api/remote/command", handler: handlers.remoteCommandHandler },
  { path: "/api/remote/poll", handler: handlers.remotePollHandler },
  { path: "/remote", handler: handlers.remotePageHandler },
];
...
const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  if (!isBrowserFile && !applyCors(req, res)) {
    // The phone remote posts same-origin from a LAN origin that can never
    // be allowlisted. It stays safe: every command must carry the pairing
    // token from the dashboard QR code, and no cookies are involved.
