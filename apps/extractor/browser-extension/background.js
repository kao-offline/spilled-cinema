  ...
  chrome.scripting.executeScript(
    {
      ...
      func: startCapturePicker,
      args: [{ appOrigin: message.appOrigin, sessionId: message.sessionId }],
    },
    () => {
...
function startCapturePicker(config) {
  const APP_ORIGIN = config.appOrigin;
  const SESSION_ID = config.sessionId;
  const OVERLAY_ID = "__spilledcinema_extension_overlay__";
  const OUTLINE_ID = "__spilledcinema_extension_outline__";
  const FRAME_MARKER = "__spilledcinema_extension_frame_listener__";
  ...
  overlay.textContent = "SpilledCinema extension: hover the player and click to capture. Esc to cancel.";
    ...
    try {
        ...
        return parentCandidatesPromise.then((parentCandidates) =>
          buildFallbackPayload(
            ...
            parentCandidates.length > 0
              ? "The iframe is cross-origin, but Svetserialu source links were extracted from the parent page."
              : "This iframe could not be inspected. Cross-origin iframe content is not accessible from the extension on this page context.",
          ),
        );
      }
      ...
      if (!nestedPlayable) {
        return parentCandidatesPromise.then((parentCandidates) => ({
          ...
          notes:
            parentCandidates.length > 0
              ? "No direct HTML5 video was found inside the iframe, but Svetserialu source links were extracted from the parent page."
      ...
      return parentCandidatesPromise.then((parentCandidates) =>
        buildFallbackPayload(
          parentCandidates,
          parentCandidates.length > 0
            ? "This iframe is cross-origin, so its DOM is blocked, but Svetserialu source links were extracted from the parent page."
  ...
  const handleMove = (sourceWindow, event) => {
    ...
    if (!target) return;
    updateOutline(pickBestTarget(sourceWindow, target, event.clientX, event.clientY), sourceWindow);
  };
  ...
  const attachListeners = (targetWindow) => {
    ...
    Array.from(targetWindow.document.querySelectorAll("iframe")).forEach((frame) => {
      try {
        ...
        // Cross-origin iframe: only the iframe shell can be selected from here.
      }
    });
  };
